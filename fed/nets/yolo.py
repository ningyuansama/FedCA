...
class YoloBody(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False):
        ...
        # ========= CMI extra parameters (class centers) ==========
        z_dim = base_channels * 4  # 与 P3/P4/P5 通道一致（256）
        self.z_dim = z_dim

        # 每个尺度生成 1x1 conv 获取 z_mu / z_logvar
        self.z_mu_convs = nn.ModuleList([
            nn.Conv2d(ch[i], z_dim, 1) for i in range(self.nl)
        ])
        self.z_logvar_convs = nn.ModuleList([
            nn.Conv2d(ch[i], z_dim, 1) for i in range(self.nl)
        ])

        # 类中心（类原型）：可学习参数 —— 跨客户端共享（FedAvg 会自动同步）
        self.class_mu = nn.Parameter(torch.randn(self.num_classes, z_dim) * 0.01)
        self.class_logvar = nn.Parameter(torch.zeros(self.num_classes, z_dim))
        # ==========================================================
    ...
    def forward(self, x,return_feat=False):
        ...
        for i in range(self.nl):
            # 原 YOLO head 输出
            out_i = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)

            # ====== produce z_mu / z_logvar ======
            z_mu_list.append(self.z_mu_convs[i](x[i]))          # 输入为P3/P4/P5，不是out_i
            z_logvar_list.append(self.z_logvar_convs[i](x[i]))
        ...
        # return dbox, cls, x, self.anchors.to(dbox.device), self.strides.to(dbox.device), z_mu_list, z_logvar_list
        
        # 在 forward() 即将返回之前，先确定 device（dbox 已经存在）
        device = dbox.device

        # 1) anchors 可能是 list/tuple 或 tensor，安全地把它搬到 device
        anchors_out = self.anchors
        if isinstance(self.anchors, (list, tuple)):
            anchors_out = [a.to(device) for a in self.anchors]
        else:
            anchors_out = self.anchors.to(device)

        # 2) strides 通常是 tensor，但也做个兼容处理
        strides_out = self.strides
        if isinstance(self.strides, (list, tuple)):
            strides_out = [s.to(device) for s in self.strides]
        else:
            strides_out = self.strides.to(device)

        # # 3) 根据 train/eval 模式返回不同的值
        # if not self.training:
        #     # eval mode: keep original 5-tuple (dbox, cls, feats, anchors, strides)
        #     return dbox, cls, x, anchors_out, strides_out

        # # training mode: return all items needed by Loss (including z_mu/z_logvar and class centers)
        return (
            dbox,
            cls,
            x,
