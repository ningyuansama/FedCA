...
def kl_diag_gaussian(mu_p, logvar_p, mu_r, logvar_r):
...
class Loss:
    ...
    def __call__(self, preds, batch):
        """
        preds: 模型预测，list of feature maps
        batch: list of Tensors，每个 Tensor 对应一张图片的 targets，shape=[num_targets, 5]
            每个 target 格式: [cls, x1, y1, x2, y2]
        """
        # device  = preds[1].device
        # loss    = torch.zeros(3, device=device)  # box, cls, dfl
        # dbox, pred_scores_raw, feats, anchor_points, stride_tensor, z_mu_list, z_logvar_list = preds
        (
            dbox,
            pred_scores_raw,
            feats,
        ...
        if len(processed_targets) == 0:
            # 如果整个 batch 都空，就返回0 loss
            return loss.sum()
        
        targets_tensor = torch.cat(processed_targets, dim=0)

        # 原来的处理逻辑
        targets = torch.cat((targets_tensor[:, 0].view(-1, 1),
                            targets_tensor[:, 1].view(-1, 1),
                            targets_tensor[:, 2:]), 1)
        ...
        for i, feat in enumerate(feats):
            B, C, H, W = feat.shape
            HW = H * W
            fg_mask_l = fg_mask[:, offset:offset + HW]  # [B, HW]

            if fg_mask_l.sum() == 0:
                offset += HW
                continue

            # flatten z_mu/z_logvar： [B, C, H, W] -> [B, HW, C]
            z_mu = z_mu_list[i].permute(0, 2, 3, 1).reshape(B, HW, -1)
            z_logv = z_logvar_list[i].permute(0, 2, 3, 1).reshape(B, HW, -1)

            # 取正样本的特征
            mu_p = z_mu[fg_mask_l]      # [N_pos, z_dim]
            logvar_p = z_logv[fg_mask_l]

            # 正样本类别
            # target_scores 是 one-hot → 取 argmax 即类别
            # 但负样本 one-hot=0，我们只取 fg_mask_l 对应的
            # cls_pos = target_scores[fg_mask_l].argmax(dim=1)  # [N_pos]
            # 1) 切当前层的 target_scores
            target_scores_l = target_scores[:, offset:offset + HW, :]   # [B, HW, nc]

            # 2) 取正样本对应的 one-hot
            ts_pos = target_scores_l[fg_mask_l]   # [N_pos, nc]

            # 3) 得到类别 index
            cls_pos = ts_pos.argmax(dim=1)        # [N_pos]


            # 类中心
            # mu_r = self.model.class_mu[cls_pos]        # [N_pos, z_dim]
            # logvar_r = self.model.class_logvar[cls_pos]
            mu_r = class_mu[cls_pos]
            logvar_r = class_logvar[cls_pos]    


            # KL
            kl = kl_diag_gaussian(mu_p, logvar_p, mu_r, logvar_r).mean()
            loss_CMI += kl

            # L2R
            loss_L2R += ((mu_p**2).sum(dim=1) + torch.exp(logvar_p).sum(dim=1)).mean()
        ...
        loss_total = loss.sum() + alpha_cmi * loss_CMI + alpha_l2r * loss_L2R
